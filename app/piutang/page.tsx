...
export default function PiutangPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const { data: debts } = await supabase
        .from('debts')
        .select(`
          id, amount, status, is_paid, created_at, note, debtor_name,
      ...
      setPiutangList((debts || []).filter(d => d.receipt !== null));
      setLoading(false);
    })();
  }, []);
  ...
  const groupByDebtor = () => {
    ...
    piutangList.forEach((debt: any) => {
      ...
      const debtorInfo = debt.debtor || { id: debtorId, name: debt.debtor_name || 'Teman Manual', email: 'Tanpa Akun' };
      if (!grouped.has(debtorId)) grouped.set(debtorId, { debtor: debtorInfo, debts: [], total: 0 });
      const g = grouped.get(debtorId)!;
      g.debts.push(debt);
      g.total += debt.amount;
    });
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    const map: Record<string, { label: string; color: string; bg: string; Icon: any }> = {
      ...
      waiting_confirmation: { label: 'Menunggu',  color: '#B07A1A', bg: '#FEF3C7', Icon: Clock        },
    };
  ...
  if (loading) return (
    <div className="min-h-screen flex flex-col items-center justify-center" style={{ background: C.bg }}>
      ...
      <p style={{ fontFamily: SFText, fontWeight: 500, fontSize: '14px', color: C.textMid }}>Memuat piutang…</p>
    </div>
  );

  const groupedData  = groupByDebtor();
  const totalPiutang = piutangList.reduce((a, c) => a + c.amount, 0);
    ...
    <div className="min-h-screen pb-20" style={{ background: C.bg, fontFamily: SF, color: C.text }}>
      <div className="max-w-[1480px] mx-auto px-6 sm:px-10 lg:px-16 pt-8">
        ...
        <div className="rounded-2xl p-6 border mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4"
          ...
          <div>
            <div className="w-8 h-1.5 rounded-full mb-3" style={{ background: C.green }} />
            <p style={{ fontFamily: SFText, fontWeight: 500, fontSize: '13px', color: C.textLt }}>Total Piutang Aktif</p>
            <p style={{ fontFamily: SF, fontWeight: 700, fontSize: '32px', color: C.green, letterSpacing: '-0.5px', marginTop: '2px' }}>
          ...
          <div className="flex gap-6">
            <div>
              ...
              <p style={{ fontFamily: SF, fontWeight: 700, fontSize: '20px', color: C.text }}>{piutangList.length}</p>
            </div>
            <div>
                ...
                <div
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold shrink-0"
                      ...
                      {group.debtor.name ? group.debtor.name[0].toUpperCase() : <User size={18} />}
                    </div>
                    <div>
                      <p style={{ fontFamily: SF, fontWeight: 700, fontSize: '15px', color: C.text }}>
                        {group.debtor.name || 'Tanpa Nama'}
                      </p>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      ...
                      <p style={{ fontFamily: SF, fontWeight: 700, fontSize: '18px', color: C.green }}>
                ...
                <div className="p-4 space-y-2">
                  {group.debts.map((debt) => (
                    <div key={debt.id}
                      ...
                      <div
                        onClick={() => debt.receipt?.id && router.push(`/receipt/${debt.receipt.id}`)}
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="truncate" style={{ fontFamily: SF, fontWeight: 600, fontSize: '14px', color: C.text }}>
                              {debt.receipt?.store_name || debt.note || 'Tagihan Manual'}
                            </p>
                          ...
                          <div className="flex items-center gap-3 mt-0.5">
                            ...
                            <p style={{ fontFamily: SF, fontWeight: 700, fontSize: '13px', color: C.green }}>
                      ...
                      {debt.status === 'pending' && debt.debtor?.id && (
                        <button
                          onClick={() => sendReminder(debt.id, debt.debtor.id, debt.receipt?.store_name || debt.note || 'Tagihan')}
                          ...
                          style={{ background: C.accent, color: C.primary, fontFamily: SF, fontWeight: 600, fontSize: '12px', border: `1.5px solid ${C.border}` }}>
                          {sending === debt.id
                            ? <><div className="w-3 h-3 border-2 rounded-full animate-spin" style={{ borderColor: C.border, borderTopColor: C.primary }} /> Mengirim…</>
                            : <><Send size={12} /> Ingatkan</>}
                        </button>
                      )}
                    </div>
                  ))}
